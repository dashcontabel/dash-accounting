...
O resumo do dashboard principal apresenta quatro divisoes em linhas completas: Receitas; Saldos Bancarios por Conta; Despesas; e Investimentos e Resultado. Receitas agrupa Faturamento, NFs Recebidas, Aluguel, Receitas Passivas e Rendimentos Liquidos; para uma selecao exclusiva da empresa PLACA, somente os dois primeiros nomes de card mudam para `Previsao` e `Recebido`. Despesas agrupa Impostos, IOF/IRRF, Pro-labores e Demais Despesas. O card `IOF_IRRF` usa os codigos mapeados, permite detalhamento no Razao e ja integra `DESPESAS_TOTAL`. Investimentos e Resultado inclui `Margem de Lucro`, exibida em percentual pela formula `RESULTADO / (FATURAMENTO + ALUGUEL + RENDIMENTO_BRUTO)`; uma base de receitas zerada deixa o indicador indisponivel. O percentual fica centralizado em um medidor circular de tamanho intermediario cuja escala visual e limitada a 100%, sem alterar o valor textual real nem retornar a altura excessiva da versao inicial. Cada cabecalho termina com seu indicador consolidado, conectado ao titulo por uma linha cromatica de destaque: `RECEITAS_TOTAL`, saldo bancario consolidado, `DESPESAS_TOTAL` e `RESULTADO`, respectivamente. Logo abaixo fica o grafico Faturamento x Despesas x Resultado. Os cards quebram dentro de cada divisao e usam apresentacao inspirada em observabilidade com numeros tabulares, status cromatico e indicador neutro animado quando ha detalhamento; o mesmo indicador e usado nos cards de Centro de Custo. O contexto de referencia/consolidacao tem destaque proprio; na selecao multiempresa, tags identificam cada empresa e permitem remove-la diretamente da consolidacao.
